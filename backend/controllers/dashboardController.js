...
exports.getDashboard = async (req, res) => {
   try {
      ...
      if (estado.includes("aceite")) {
         propostaStats.aceite += row.total;
      } else if (estado.includes("pendente")) {
         propostaStats.pendente += row.total;
      } else if (estado.includes("recusada") ) {
         propostaStats.recusada += row.total;
      } else {
         console.warn("Estado desconhecido:", row.estado);
      }
      });
